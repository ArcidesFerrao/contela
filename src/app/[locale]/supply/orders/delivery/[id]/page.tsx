...
import { ListDeliveryItem } from "@/components/ListDeliveryItem";
      ...
      <div className="stats p-4 flex flex-col gap-3">
        ...
        <div className="grid gap-2 md:hidden">
          {delivery.deliveryItems.map((item) => (
            <ListDeliveryItem
              key={item.id}
              id={item.orderItem.stockItemId}
              locale={locale}
              name={item.orderItem.stockItem.name}
              price={item.orderItem.price}
              qty={item.quantity}
            />
          ))}
        </div>
        <table className="w-full text-sm delivery-items-table sm:hidden md:table ">
          <thead>
